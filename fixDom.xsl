...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
	...
	xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:px="http://www.publishingx.de"
	exclude-result-prefixes="xs px xsi" version="2.0">
	...
	<!-- Add app property to global Object	-->
	<xsl:template match="classdef[@name = 'global'][ancestor::js]/elements">
		<xsl:copy exclude-result-prefixes="#all" inherit-namespaces="no" copy-namespaces="no">
			<xsl:copy-of select="@*"/>
					...
					<xsl:for-each select="tokenize(px:cleanAcceptReturn(regex-group(2)), ',')">
						...
						<xsl:if test="replace(., '\s', '') != ''">
							<datatype>
								...
								<xsl:if
									test="not(starts-with(px:cleanTypeName(., $datatypeNode), 'NothingEnum'))">
									<xsl:choose>
										<xsl:when test="$array">
											<xsl:copy-of select="$array"/>
										</xsl:when>
				...
				<property name="parent">
					...
					<xsl:analyze-string select="shortdesc" regex="^The parent.+?\(a (.+?)\)">
						<xsl:matching-substring>
							...
							<xsl:for-each
								select="tokenize(replace(regex-group(1), ' or ', ','), ',')">
								<xsl:sort/>
								<datatype>
									<type>
	...
	<xsl:template name="checkForRange">
		...
		<xsl:if
			test="not($datatypeNode/min and $datatypeNode/max) and $datatypeNode/preceding-sibling::shortdesc">
			<xsl:analyze-string select="$datatypeNode/preceding-sibling::shortdesc"
				regex="(range of|Range:) (\d+) to (\d+)">
				<xsl:matching-substring>
					<xsl:comment>
						<xsl:value-of select="."/>
					</xsl:comment>
	...
	<xsl:function name="px:cleanTypeName">
		...
		<xsl:variable name="clean0">
			...
			<xsl:if
				test="$context/ancestor::sui and not(matches($text, '(object|string|bool|number|array|function|file|folder)', 'i'))">
				<xsl:text>SUI</xsl:text>
			</xsl:if>
		...
		<xsl:choose>
			...
			<xsl:otherwise>
				...
				<xsl:variable name="clean3"
					select="concat(upper-case(substring($clean2, 1, 1)), substring($clean2, 2))"/>
				...
				<xsl:variable name="clean98"
					select="replace($clean5, 'NothingEnumerat', 'NothingEnum')"/>
				...
				<xsl:variable name="clean101"
					select="replace($clean100, 'SpecialCharacterss', 'SpecialCharacters')"/>
