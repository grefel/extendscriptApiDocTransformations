...
You can find an alternative node.js approach [at](https://github.com/yearbookmachine/extendscript-api-documentation). There a also HTML compilations from [jongware](http://www.jongware.com/idjshelp.html).
...
1. Merge the Source files and get rid of namespace bugs with `mergeFiles.xslt`.  This transformation works with a named template (Saxon Option is '-it mergeDOMFiles'). You can change the path params in the file, or call the transformation with the the path to indesign.xml, javascript.xml scriptui.xml.
