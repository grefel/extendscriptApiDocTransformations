...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
	...
	xmlns:omv="http://schema.adobe.com/omv/1.0/omv.xsd" exclude-result-prefixes="xs omv xsi"
	version="2.0">
	...
	<xsl:param name="product.xml">sourceXML/id_26.xml</xsl:param>
	...
	<xsl:template name="mergeDOMFiles">
		<xsl:if
			test="not(doc-available($javascript.xml) or doc-available($product.xml) or doc-available($scriptui.xml))">
			<xsl:message>
				<xsl:text>[Caution] Not all Files are available! </xsl:text>
				<xsl:value-of select="$product.xml"/>
				<xsl:text>, </xsl:text>
				<xsl:value-of select="$javascript.xml"/>
				<xsl:text>, </xsl:text>
		...
		<domRoot>
			...
			<xsl:if test="doc-available($javascript.xml)">
				<js>
					<xsl:apply-templates select="document($javascript.xml)"/>
				</js>
			</xsl:if>
			<xsl:if test="doc-available($scriptui.xml)">
				<sui>
					<xsl:apply-templates select="document($scriptui.xml)"/>
				</sui>
			</xsl:if>
			<xsl:if test="doc-available($product.xml)">
				<product>
